...
class MapHandlers(RequestHandler):
    def get(self, h):  
        ...
        if h == "index":  
            ...
            resp_data['list'] = list #resp_data['list'] 是小车基础信息数据
        ...
        if h == 'data': #接口，用来给前端js提供数据
            ...
            print(123)
            resp_data3['list'] = list #resp_data['list'] 是小车基础信息数据
...
#             self.write(json_str)  # 将resp作为HTTP响应返回
        ...
        if h == "api":  # 如果h等于"api"
            ...
            if workstate == "2" and image:
                ...
                print(image)
                self.application.db.insert("INSERT INTO yichao (longitude, latitude, image, is_sanitized) VALUES (%s, %s, '%s', %s)"%(longitude, latitude, image, is_sanitized))
                print("图片保存成功")
            print("successfully")
