...
class LoginHandlers(RequestHandler):
    ...
    def post(self):
        ...
        # 获取用户的登录名称和登陆密码
        login_name = self.get_body_argument("login_name", default='')
        login_pwd = self.get_body_argument("login_pwd", default='')
        print(login_pwd)
        print(login_name)
        # 进行参数有效性的校验
...
class IndexHandlers(RequestHandler):
    ...
    def command(self):
        ...
        res = yield client.fetch("http://127.0.0.1:8000/hardware")
        if res.error:
            ret = {"ret": 0}
        else:
            ret = "successfully"
